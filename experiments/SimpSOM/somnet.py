...
from experiments import SimpSOM as hx, SimpSOM as dp, SimpSOM as qt

from .somnode import SOMNode as somNode
...
class SOMNet:
    ...
    def train(self, startLearnRate=0.01, epochs=-1):
        ...
        for i in range(self.epochs):
            ...
            for node in self.nodeList:
                node.update_weights_hamming(inputVec, self.sigma, self.lrate, bmu)
    ...
    def nodes_graph(self, colnum=0, show=False, printout=True):
        ...
        if self.colorEx == True:
            ...
            ax = hx.plot_hex(fig, centers, cols)
            ax.set_title('Node Grid w Feature #' + str(colnum), size=10)
            divider = make_axes_locatable(ax)
            cax = divider.append_axes("right", size="5%", pad=0.0)
            cbar = plt.colorbar(ax.collections[0], cax=cax)
            cbar.set_label('Feature #' + str(colnum) + ' value', size=10, labelpad=5)
    ...
    def diff_graph(self, show=False, printout=True, returns=False, useGraph=None):
        ...
        for node, neighbours in zip(self.nodeList, neighbours):
            ...
            for nb in neighbours:
                diff = diff + node.get_distance_hamming(nb.weights)
            diffs.append(diff)
        ...
        if show == True or printout == True:
            ...
            ax = hx.plot_hex(useGraph, centers, diffs)
            ax.set_title('Nodes Grid w Weights Difference', size=10)

            divider = make_axes_locatable(ax)
            cax = divider.append_axes("right", size="5%", pad=0.0)
            cbar = plt.colorbar(ax.collections[0], cax=cax)
            cbar.set_label('Weights Difference', size=10, labelpad=20)
    ...
    def cluster(self, array, type='qthresh', cutoff=5, quant=0.2, percent=0.02, numcl=8, \
                savefile=True, filetype='dat', show=False, printout=True, useGraph = None):
